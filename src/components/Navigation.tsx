  ...
  return (
    ...
    >
      <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 sm:px-6 lg:px-8">
        <a href="#home" className="flex items-center gap-2 text-sm font-semibold tracking-tight text-slate-950 dark:text-white">
          <span className="rounded-lg border border-teal-500/30 bg-teal-500/10 px-2.5 py-1 font-mono text-teal-700 dark:text-teal-300">&lt;/&gt;</span>
          <span>Bilal</span>
        </a>
      ...
      {isOpen && (
        <div className="border-t border-slate-200 bg-white/96 px-4 py-3 backdrop-blur-xl dark:border-white/10 dark:bg-[#0B1220]/96 lg:hidden">
          <div className="mx-auto grid max-w-7xl gap-1">
            {navItems.map((item) => (
              <a
                ...
                className="rounded-lg px-3 py-3 text-sm font-medium text-slate-700 transition hover:bg-slate-100 hover:text-slate-950 dark:text-slate-300 dark:hover:bg-white/5 dark:hover:text-white"
              >
                {item.name}
              </a>
            ))}
