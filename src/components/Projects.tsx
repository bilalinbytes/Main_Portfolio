        ...
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {projects.map((project) => (
            <article key={project.title} className="premium-card flex flex-col">
              <div className="p-6">
                <h3 className="text-main text-xl font-semibold">{project.title}</h3>
                <p className="text-copy-muted mt-3 min-h-28 text-sm leading-6">{project.description}</p>
                <div className="mt-5 flex flex-wrap gap-2">
                  {project.focus.map((tech) => (
                    <span key={tech} className="rounded-md border border-teal-500/20 bg-teal-500/10 px-2.5 py-1 text-xs font-medium text-teal-800 dark:text-teal-100">
                      {tech}
                    </span>
                  ))}
                </div>
              ...
              <div className="mt-auto flex flex-wrap gap-2 px-6 pb-6">
                <a href={project.github} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-lg border border-slate-200 px-3 py-2 text-sm font-medium text-slate-700 transition hover:border-teal-500/50 hover:text-slate-950 dark:border-white/10 dark:text-slate-200 dark:hover:text-white">
                  <Github size={16} />
                  Code
                </a>
                {project.demo && (
                  <a href={project.demo} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-lg bg-teal-700 px-3 py-2 text-sm font-medium text-white transition hover:bg-teal-600">
                    <ExternalLink size={16} />
                    Live
                  </a>
                )}
                {project.certificate && (
                  <a href={project.certificate} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-lg border border-teal-500/20 bg-teal-500/10 px-3 py-2 text-sm font-medium text-teal-800 transition hover:border-teal-500/50 dark:text-teal-100">
                    <Award size={16} />
                    Certificate
                  </a>
                )}
